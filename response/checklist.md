...
| Integrate existing internal data sets (Excel/CSV uploads, API, database linkages) | Must Have | ✅ Yes | CSV upload interface, PostgreSQL data warehouse, future Fluxx/DMP API integration | Section 3.3 | Proposal Week 1-2 deliverable |
...
| Spatial Queries – identify projects within selected polygons | Good to Have | ✅ Yes | PostGIS spatial queries, "show all projects in aspirational districts" | Section 3.1 | PostGIS ST_Contains queries |
...
| API / CSV / Excel Uploads – manual and automated imports | Must Have | ✅ Yes | Web upload form, drag-drop, validation, PostgreSQL ingestion | Section 3.3 | CSV upload interface (Week 3) |
| Data stabilization – standardize codes (districts/blocks), merge datasets | Must Have | ✅ Yes | Geocoding pipeline, district name normalization, PostGIS matching | Section 3.3 | Data pipeline scripts (Week 2) |
...
| Authentication & Encryption – secure login, data transfer | Must Have | ✅ Yes | HTTPS/TLS 1.3, AES-256 at rest, HTTP Basic Auth (Phase 1) | Section 7.3 | Security implementation (Week 1) |
| Scalable Infrastructure – cloud hosting, load balancing | Good to Have | ✅ Yes | AWS (or Azure), auto-scaling EC2, RDS, CDN (CloudFront) | Section 2.3 | Cloud setup (Week 1) |
| **Maintenance & Support** |
...
| Data backup – periodic backup of data | Must Have | ✅ Yes | Daily automated RDS backups, S3 versioning, 30-day retention | Section 7.3 | Backup automation (Week 1) |
...
| Geo-tagged Media Integration – display photos/videos from project sites | Good to Have | ⏳ Phase 2 | Phase 2: Media storage (S3), geo-tagging, map popups | Section 8.2 | Phase 2 media integration |
...
| c) Data integration (APIs, ETL, SQL/NoSQL, shapefiles, GeoJSON, KML) | 4.c | ✅ Yes | Section 2.3, 3.3 |
...
|-------------|------------------|----------|
| **Functional Requirements (OOB / Minimal Customization)** | All must-have requirements delivered OOB with Observable Plot, Mapbox, PostgreSQL | Functional Requirement Sheet (above) shows 100% must-have compliance |
...
| Database (SQL/NoSQL) | PostgreSQL with PostGIS (open-source, proven for GIS) | Section 2.3 |
| Cloud hosting | AWS (or Azure per client preference) | Section 2.3 |
...
| **Development** | Observable Plot charts, Mapbox maps, PostgreSQL data layer | Section 2.3, 4.2 |
| **Testing** | UAT (Week 7), accessibility audit, performance testing | Section 4.2 (Week 7) |
| **Deployment** | AWS cloud, CI/CD pipeline, monitoring setup | Section 4.2 (Week 8) |
...
| Senior Developer | 8 years | JavaScript, D3.js, Observable Plot, React, responsive design | Section 5.1 |
| Data Engineer | 6 years | PostgreSQL, PostGIS, ETL, Python, data validation | Section 5.1 |
| GIS Specialist | 5 years | Mapbox, Leaflet, QGIS, spatial analysis, shapefiles | Section 5.1 |
| DevOps Engineer | 7 years | AWS, CI/CD, monitoring, infrastructure as code | Section 5.1 |
...
4. **Cloud Provider**: AWS approved (or Azure specified by Week 0).
