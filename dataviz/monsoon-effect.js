  ...
  async function init() {
    ...
    try {
      ...
      const avgWithPreDropout = withPreMonsoon.length > 0
        ? withPreMonsoon.reduce((sum, d) => sum + d.dropout_rate, 0) / withPreMonsoon.length
        : 0;
      const avgWithoutPreDropout = withoutPreMonsoon.length > 0
        ? withoutPreMonsoon.reduce((sum, d) => sum + d.dropout_rate, 0) / withoutPreMonsoon.length
        : 0;
      ...
      subtitle.innerHTML = `Monsoon reduces outcomes by ${
        formatNumber(Math.abs(outcomeDrop), 1)
      }%, but pre-monsoon outreach cuts dropout by ${formatPercent(dropoutLift, 1)}.`;
