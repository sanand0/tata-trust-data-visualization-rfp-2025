...
import { COLORS, formatNumber, formatPercent, groupBy, loadData } from "./core.js";
...
export default function render(container, props = {}) {
  ...
  async function init() {
    ...
    try {
      ...
      const cap2Variance = grouped[2]
        ? grouped[2].reduce((sum, d) => sum + Math.abs(d.budget_variance_pct), 0) / grouped[2].length
        : 0;
      const cap4Variance = grouped[4]
        ? grouped[4].reduce((sum, d) => sum + Math.abs(d.budget_variance_pct), 0) / grouped[4].length
        : 0;
      ...
      summary.innerHTML = `Budget variance drops <strong>${
        formatPercent(varianceReduction, 0)
      }</strong> from capacity 2 to 4. Box plots show median and quartiles.`;
