...
import { loadData, groupBy, sumBy, meanBy, formatNumber, formatPercent, COLORS } from "./core.js";
  ...
  async function init() {
    ...
    try {
      ...
      const partnerScores = Object.entries(grouped).map(([name, grants]) => {
        const efficiency = sumBy(grants, d => (d.outcome_index * d.beneficiaries)) / (sumBy(grants, 'budget_m_inr') * 1000000);
      ...
      subtitle.innerHTML = `Top 10 and bottom 10 partners by combined performance. Green = strong, red = needs attention.`;
