...
import { loadData, meanBy, formatNumber, COLORS } from "./core.js";
  ...
  async function init() {
    ...
    try {
      ...
      const confidenceData = [
        { group: 'On-time disbursements', lower: chartData[0].outcome - 1.96 * onTimeSE, upper: chartData[0].outcome + 1.96 * onTimeSE },
        { group: 'Delayed disbursements', lower: chartData[1].outcome - 1.96 * delayedSE, upper: chartData[1].outcome + 1.96 * delayedSE }
      ];
      ...
      chart = Plot.plot({
        ...
        marks: [
          ...
          Plot.barY(chartData, {
            ...
            tip: true,
            title: d => `${d.group}\nAvg Outcome: ${formatNumber(d.outcome, 1)}\nGrants: ${d.count}\nAvg Timeliness: ${formatNumber(d.timeliness * 100, 0)}%`
          }),
      ...
      insight.innerHTML = `On-time disbursements achieve <strong>${formatNumber(outcomeDiff, 1)} points</strong> higher outcomes (${formatNumber(pctDiff, 0)}% improvement). Bars show 95% confidence intervals.`;
